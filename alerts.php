#!/usr/bin/env php
<?php
/* Copyright (C) 2014-2015 Daniel Preussker <f0o@devilcode.org>
 ...
 * @copyright 2014-2015 f0o, LibreNMS
 * @license GPL
 * @package LibreNMS
 * @subpackage Alerts
 */
class Dispatcher extends Alert {
	...
	final private function RunFollowUp() {
		...
		foreach( dbFetchRows("SELECT alerts.device_id, alerts.rule_id, alerts.state FROM alerts,alert_rules WHERE alerts.rule_id = alert_rules.id && alert_rules.disabled = 0 && alerts.state != 2 && alerts.open = 0") as $alert ) {
			...
			$chk = dbFetchRows($this->GenSQL($alert['rule']),array($alert['device_id']));
		...
		foreach( dbFetchRows("SELECT alerts.device_id, alerts.rule_id, alerts.state FROM alerts,alert_rules WHERE alerts.rule_id = alert_rules.id && alert_rules.disabled = 0 && alerts.state != 2 && alerts.open = 1") as $alert ) {
			...
			if( !$noiss ) {
				...
				if( is_array($obj) ) {
					...
					$msg = $this->FormatAlertTpl($tpl,$obj);
					$obj['msg'] = $msg;
	 ...
	 * @return void
	 */
	final private function ExtTransports($obj) {
		global $config;
	 ...
	 * @return string
	 */
	final private function FormatAlertTpl($tpl,$obj) {
		...
		while( ++$x < $s ) {
			...
			if( $pos != -1 && $msg[$x] == "}" ) {
				...
				$cond   = trim($this->populate(substr($orig,$o,-1),false));
				$native = $native[0].$cond.$native[1];
				$parsed = str_replace($orig,$native,$parsed);
				unset($cond, $o, $orig, $native);
			}
		}
		$parsed = $this->populate($parsed);
		return $this->RunJail($parsed,$obj);
	}
	 ...
	 * @return array
	 */
	final private function DescribeAlert($alert) {
		$obj = array();
		$i = 0;
	 ...
	 * @return string
	 */
	final private function TimeFormat($secs){
		$bit = array(
			'y' => $secs / 31556926 % 12,
			'w' => $secs / 604800 % 52,
			'd' => $secs / 86400 % 7,
	 ...
	 * @return string
	 */
	final private function populate($txt,$wrap=true) {
