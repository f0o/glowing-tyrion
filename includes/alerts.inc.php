...
class Alert {
	 ...
	 * @return string
	 */
	final protected function GenSQL($rule) {
		$tmp = explode(" ",$rule);
		$tables = array();
		foreach( $tmp as $opt ) {
		...
		foreach( dbFetchRows("SELECT * FROM alert_rules WHERE alert_rules.disabled = 0 && ( alert_rules.device_id = -1 || alert_rules.device_id = ? ) ORDER BY device_id,id",array($device)) as $rule ) {
			...
			if( $doalert ) {
				if( $chk['state'] === "2" ) {
					...
					$extra = gzcompress(json_encode(array('contacts' => $this->GetContacts($qry), 'rule'=>$qry)),9);
	 ...
	 * @return array
	 */
	protected function GetContacts($results) {
		global $config;
		if( sizeof($results) == 0 ) {
			return array();
		}
